...
def Train_adv_network(model, network, device, train_loader, k, dmin, date):
    ...
    for data in train_loader:
        ...
        for Epoch in range(2000):  #
            ...
            if abs(max(second_loss)-loss_temp) <= 0.000001 and d_new <= 5*dmin:  #控制阈值，使其更加大，以产生更多满足原始阈值的数据，提高准确率
                d_new = d_new*1.05
            if Epoch > 100 and max(second_loss) <= 0.1 and max(third_loss) <= 0.1:  # 动态改变权重。前期可将alpha=0.1，重要优化攻击精度。精度达到上限之后，逐渐增大alpha，是的gamma更加平滑
                break
...
model = torch.load('../online/conv_white/ConvCNN_white.pth', map_location=torch.device('cpu'))
CNN = Generator.Generator()

errors90_all = pickle.load(open("../online/conv_white/ConvCNN_white_meta_error90_info.pkl", 'rb'))
date = 0.15

Errs_k_b = np.empty((1, 1+500))
Errs_k_a = np.empty((1, 1+500))
Accs_b = np.empty((1, 1+1))
Accs_a = np.empty((1, 1+1))
Adv_weights = np.empty((1, 1+52))
...
for k in np.arange(num_classes):
    ...
    d_min = errors90_all[k] + 0.3

    # network = torch.load('../online/adv_conv_white/ut_adv_white_conv' + '%d-' % k + '.pth')
    network = Train_adv_network(model, CNN, device, dataloader_train, k, d_min, date)
    _, err_k_b, err_k_a, final_acc_b, final_acc_a, adv_weight, loc_prediction_b, loc_prediction_a = Test_adv_network(model, network, device, dataloader_test, k, d_min, date)
...
print('Before Error_k 0.5 & 0.9: %.5f & %.5f' % (np.quantile(Errs_k_b[:, 1:501], 0.5), np.quantile(Errs_k_b[:, 1:501], 0.9)))
print('After Error_k 0.5 & 0.9: %.5f & %.5f' % (np.quantile(Errs_k_a[:, 1:501], 0.5), np.quantile(Errs_k_a[:, 1:501], 0.9)))


file_name = '../online/conv_white/ut_Attack_Results_all_conv_white.mat'
savemat(file_name, {'Errors_k_b': Errs_k_b, 'Errors_k_a': Errs_k_a, 'Accuracy_before': Accs_b, 'Accuracy_after': Accs_a, 'Adv_weights': Adv_weights, 'Perdiction_b': Perdiction_b, 'Perdiction_a': Perdiction_a})