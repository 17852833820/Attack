...
class T_offine_conv_white():
    def __init__(self):
        ...
        self.date = 0.1
        self.d_max = 0.75
        self.Prediction_a=np.array([])
        self.Prediction_b=np.array([])
        ...
        self.writer= SummaryWriter('./logs/trainGAN/CNN/{0}/tensorboard'.format(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))))
    ...
    def Train_adv_network(self,model, network, device, train_loader, k, n, dmax, date):
        ...
        for data in train_loader:
            ...
            for Epoch in range(10000):  #
                ...
                if abs(max(first_loss)-loss_temp) <= 0.0001 and d_new >= dmax/5:
                    d_new = d_new/1.05
                if Epoch > 500 and max(first_loss) <= 0.1 and max(third_loss) <= 0.05:
                    break
                loss_temp = max(first_loss)

        if isinstance(network, torch.nn.DataParallel):
            torch.save(network.module, '../offline/adv_conv_white/adv_white_conv_new' + '%d-' % k + '%d' % n + '.pth')
        else:
            torch.save(network, '../offline/adv_conv_white/adv_white_conv_new' + '%d-' % k + '%d' % n + '.pth')
        return network
    ...
    def Test_adv_network(self,model, network, device, test_loader, k, n, dmax, date):  # model: Loc model, CNN: perturbation model, K: original location, n: targeted location, dmax：threshold
        ...
        with torch.no_grad():
            for data in test_loader:
                ...
                loc_prediction_a=np.append(loc_prediction_a,predict.cpu())
    ...
    def run(self):
        for k in np.arange(self.num_classes):
            ...
            for n in list_k:
                network = torch.load('../offline/adv_conv_white/adv_white_conv_new' + '%d-' % k + '%d' % n + '.pth',map_location=torch.device('cpu'))
                network = self.Train_adv_network(self.model, network, self.device, dataloader_train, k, n, self.d_max, self.date)
                _, _, err_k_b, err_k_a, err_n_b, err_n_a, final_acc_b, final_acc_a, adv_weight ,loc_prediction_b ,loc_prediction_a= self.Test_adv_network(self.model, network, self.device, dataloader_test, k, n, self.d_max, self.date)
                ...
                self.Prediction_a=np.append(self.Prediction_a,np.array([np.concatenate((np.array([k,n]),loc_prediction_a))]),axis=0)
                self.Prediction_b=np.append(self.Prediction_b,np.array([np.concatenate((np.array([k,n]),loc_prediction_b))]),axis=0)
