...
for k in np.arange(num_classes):
    ...
    for n in list_k:
        network = torch.load('../offline/adv_conv_white/adv_white_conv_new' + '%d-' % k + '%d' % n + '.pth')
        network = Train_adv_network(model, network, device, dataloader_train, k, n, d_max, date)
