...
def Train_loc(model, dataloader_train, device, num_epochs=200):
    ...
    if isinstance(model, torch.nn.DataParallel):
        torch.save(model.module, '../offline/fcnn_white/FCNN_white.pth')
    else:
        torch.save(model, '../offline/fcnn_white/FCNN_white.pth')
...
def Test_loc(model, device, testdatapath, Num_classes):
    ...
    pickle.dump(errs_all, open("../offline/fcnn_white/FCNN_white_meta_error_all_info.pkl", "wb"))
    pickle.dump(errs_90_all, open("../offline/fcnn_white/FCNN_white_meta_error90_info.pkl", "wb"))
...
network = torch.load('../offline/fcnn_white/FCNN_white.pth')

path_train = '../datas/Offline_A_up_SIMO.csv'
device = torch.device("cuda:2" if torch.cuda.is_available() else "cpu")
print(device)
network = torch.nn.DataParallel(network, device_ids=[2, 1, 3, 0])
