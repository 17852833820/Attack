...
def Train_adv_network(model, network, device, train_loader, k, dmin, date):
    ...
    if isinstance(network, torch.nn.DataParallel):
        torch.save(network.module, '../offline/adv_conv_white/ut_adv_white_conv' + '%d-' % k + '.pth')
    else:
        torch.save(network, '../offline/adv_conv_white/ut_adv_white_conv' + '%d-' % k + '.pth')
    return network
...
for k in np.arange(num_classes):
    ...
    network = torch.load('../offline/adv_conv_white/ut_adv_white_conv' + '%d-' % k + '.pth')
    network = Train_adv_network(model, network, device, dataloader_train, k, d_min+0.2, date)
    _, err_k_b, err_k_a, final_acc_b, final_acc_a, adv_weight = Test_adv_network(model, network, device, dataloader_test, k, d_min, date)
...
file_name = '../../offline/conv_white/ut_Attack_Results_all_conv_white.mat'
