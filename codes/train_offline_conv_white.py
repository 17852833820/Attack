...
class DNN_offine_conv_white():
    def __init__(self):
        ...
        self.network = torch.load('../online_new/conv_white/ConvCNN_white.pth')#DNNA pre-trained
        ...
        self.writer= SummaryWriter('../logs/trainDNN/CNN/white/2022-07-01-16-38-07/tensorboard')
        ...
        for Epoch in range(num_epochs):
            ...
            if Epoch%50==0:
                if isinstance(model, torch.nn.DataParallel):
                    torch.save(model.module, '../online_new/conv_white/ConvCNN_white{0}.pth'.format(Epoch))
                else:
                    torch.save(model, '../online_new/conv_white/ConvCNN_white{0}.pth'.format(Epoch))
        print('Finished Training')
        if isinstance(model, torch.nn.DataParallel):
            torch.save(model.module, '../online_new/conv_white/ConvCNN_white.pth')
        else:
            torch.save(model, '../online_new/conv_white/ConvCNN_white.pth')
    ...
    def Test_loc(self,model, device, testdatapath, Num_classes):
        ...
        pickle.dump(errs_all, open("../online_new/conv_white/ConvCNN_white_meta_error_all_info.pkl", "wb"))
        pickle.dump(errs_90_all, open("../online_new/conv_white/ConvCNN_white_meta_error90_info.pkl", "wb"))
    ...
    def run(self):
        ...
        model = torch.load('../online_new/conv_white/ConvCNN_white.pth')
        model = model.double()
