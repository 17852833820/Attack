...
class DNN_offine_conv_black():
        ...
        for Epoch in range(num_epochs):
            ...
            if Epoch%100==0:
                if isinstance(model, torch.nn.DataParallel):
                    torch.save(model.module, '../online_new/conv_black/ConvCNN_black.pth')
                else:
                    torch.save(model, '../online_new/conv_black/ConvCNN_black.pth')
    ...
    def Test_loc(self,model, device, testdatapath, Num_classes):
        ...
        pickle.dump(errs_all, open("../online_new/conv_black/ConvCNN_black_meta_error_all_info.pkl", "wb"))
        pickle.dump(errs_90_all, open("../online_new/conv_black/ConvCNN_black_meta_error90_info.pkl", "wb"))
    ...
    def run(self):
        ...
        model = torch.load('../online_new/conv_black/ConvCNN_black.pth')
        model = model.double()
