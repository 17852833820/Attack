...
def Train_adv_network(model, network, device, train_loader, k, n, dmax, date):
    ...
    optimizer = optim.SGD(network.parameters(), lr=0.025, momentum=0.7)
    ...
    if isinstance(network, torch.nn.DataParallel):
        torch.save(network.module, '../offline/adv_fcnn_white/adv_white_fcnn_best1' + '%d-' % k + '%d' % n + '.pth')
    else:
        torch.save(network, '../offline/adv_fcnn_white/adv_white_fcnn_best1' + '%d-' % k + '%d' % n + '.pth')
    return network
...
errors90_all = pickle.load(open("../offline/fcnn_white/FCNN_White_best1_meta_error90_info.pkl", 'rb'))
date = 0.2
d_max = 0.75
...
file_name = 'Attack_Results_all_fcnn_white_best1.mat'
