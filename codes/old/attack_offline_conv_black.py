...
def Train_adv_network(model, network, device, train_loader, k, n, dmax, date):
    ...
    if isinstance(network, torch.nn.DataParallel):
        torch.save(network.module, '../offline/adv_conv_black/adv_black_conv' + '%d-' % k + '%d' % n + '.pth')
    else:
        torch.save(network, '../offline/adv_conv_black/adv_black_conv' + '%d-' % k + '%d' % n + '.pth')
    return network
...
model_surrogate = torch.load('../offline/fcnn_black/FCNN_black.pth') # for training adversarial weights
model_victim = torch.load('../offline/conv_white/ConvCNN_white.pth') # victim model
...
for k in np.arange(num_classes):
    ...
    for n in list_k:
        # network = torch.load('../offline/adv_conv_black/adv_black_conv' + '%d-' % k + '%d' % n + '.pth')
        network = Train_adv_network(model_surrogate, CNN, device, dataloader_train, k, n, d_max-0.1, date)
...
file_name = '../offline/conv_black/Attack_Results_all_conv_black.mat'
