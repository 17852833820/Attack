...
def plot_CSI(data, data_per, ave0, ave1):
    ...
    dataframe.to_csv("../ON_RESULTS/FCNN_inputs_targeted[%d].csv" % (k))
    ...
    dataframe1.to_csv("../ON_RESULTS/FCNN1_targeted[%d].csv"% (k))
    dataframe2 = DataFrame(d2)
    dataframe2.to_csv("../ON_RESULTS/FCNN2_targeted[%d].csv"% (k))
...
for k in range(Num_classes):
    ...
    W_sets[k] = []
    #CNN=torch.load( "../ON_RESULTS/modelConvCNN_untargeted_"+'%d'%k+".pth")
    ...
    for Epoch in range(MAX_ITERATIONS):
        ...
        for Data in dataloader_attack:
            ...
            reg = torch.ones(1, 52).requires_grad_().to(device)
            for param in CNN.parameters():
                reg = torch.norm(param - torch.ones(1, 52).to(device), p=2) - date
            # reg = torch.norm(data_per.div(inputs)-torch.ones(2,56)) - date
            loss2 = (nn.ReLU()(reg))
            #loss3 = torch.sum(torch.diff(param))
            #loss3=nn.ReLU()(torch.Tensor(lam1*(1.2-float(np.max(param.tolist()))/float(np.min(param.tolist())))))
            lam = (loss1)/ nn.Sigmoid()(loss2)
            loss = loss1 + (lam+0.1) * loss2 #+ loss3
    ...
    W_sets[k] = param
    torch.save(CNN,"../ON_RESULTS/modelFCNN_targeted_"+'%d'%k+".pth")
    for param in CNN.parameters():
        W_sets[k] = param

        save_per.append(param.tolist())
    path_test='../datas/B_UP_ALL.csv'
...
dataframe.to_csv("../ON_RESULTS/FCNN_per_targeted.csv" )
...
for i, j in zip(x, y):
        ...
        if j - 0.9 > 0.01:
            key2_all.append(i)
    # plt.step(x, y)
plt.savefig("../ON_RESULTS/ConvCNN_err_targted_all.png")
plt.show()
...
for i, j in zip(x, y):
        ...
        if j - 0.9 > 0.01:
            key4_all.append(i)
    # plt.step(x, y)
plt.savefig("../ON_RESULTS/ConvCNN_acc_targted_all.png")
plt.show()
...
pickle.dump(results, open("../ON_RESULTS/ConvCNN_results_targeted.pkl", "wb"))
