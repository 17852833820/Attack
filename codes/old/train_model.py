...
def Train_loc(model, dataloader_train, device, num_epochs=200):
    ...
    optimizer = optim.SGD(model.parameters(), lr=0.4, momentum=0.7)
    ...
    if isinstance(model, torch.nn.DataParallel):
        torch.save(model.module, '../offline/fcnn_white/FCNN_white_new.pth')
    else:
        torch.save(model, '../offline/fcnn_white/FCNN_white_new.pth')
...
def Test_loc(model, device, testdatapath, Num_classes):
    ...
    for k in range(Num_classes):
        ...
        dataloader = torch.utils.data.DataLoader(dataset, batch_size=1000, shuffle=False, num_workers=16, pin_memory=True)
    ...
    pickle.dump(errs_all, open("../offline/fcnn_white/FCNN_White_new_meta_error_all_info.pkl", "wb"))
    pickle.dump(errs_90_all, open("../offline/fcnn_white/FCNN_White_new_meta_error90_info.pkl", "wb"))
...
print(device)
network = torch.nn.DataParallel(network, device_ids=[3, 0, 1, 2])
