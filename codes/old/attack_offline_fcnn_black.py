...
def Train_adv_network(model, network, device, train_loader, k, n, dmax, date):
    ...
    if isinstance(network, torch.nn.DataParallel):
        torch.save(network.module, '../offline/adv_fcnn_black/adv_black_fcnn' + '%d-' % k + '%d' % n + '.pth')
    else:
        torch.save(network, '../offline/adv_fcnn_black/adv_black_fcnn' + '%d-' % k + '%d' % n + '.pth')
    return network
...
model_surrogate = torch.load('../offline/conv_black/ConvCNN_black.pth') # for training adversarial weights
model_victim = torch.load('../offline/fcnn_white/FCNN_white.pth') # victim model
# model_surrogate = torch.nn.DataParallel(model_surrogate, device_ids=[0, 2, 3])
# model_victim = torch.nn.DataParallel(model_victim, device_ids=[0, 2, 3])
...
for k in np.arange(num_classes):
    ...
    dataloader_test = torch.utils.data.DataLoader(data_test, batch_size=250, shuffle=True, num_workers=8, pin_memory=True)
    threshold_k = errors90_all[k] + d_max
    list_k = pairing(k, threshold_k)
    for n in list_k:
        network = torch.load('../offline/adv_fcnn_black/adv_black_fcnn' + '%d-' % k + '%d' % n + '.pth')
        network = Train_adv_network(model_surrogate, network, device, dataloader_train, k, n, d_max-0.1, date)
        _, _, err_k_b, err_k_a, err_n_b, err_n_a, final_acc_b, final_acc_a, adv_weight = Test_adv_network(model_victim, network, device, dataloader_test, k, n, d_max, date)
...
file_name = '../offline/fcnn_black/Attack_Results_all_fcnn_black.mat'
