...
def Train_adv_network(model, network, device, train_loader, k, dmin, date):
    ...
    for data in train_loader:
        ...
        for Epoch in range(2000):  #
            ...
            # if abs(max(second_loss)-loss_temp) <= 0.0000001 and d_new <= 3*dmin:
            #     d_new = d_new*1.05
    ...
    if isinstance(network, torch.nn.DataParallel):
        torch.save(network.module, '../offline/adv_conv_white/ut_adv_white_conv_new' + '%d-' % k + '.pth')
    else:
        torch.save(network, '../offline/adv_conv_white/ut_adv_white_conv_new' + '%d-' % k + '.pth')
    return network
...
for k in np.arange(num_classes):
    ...
    network = torch.load('../offline/adv_conv_white/ut_adv_white_conv_new' + '%d-' % k + '.pth')
    network = Train_adv_network(model, network, device, dataloader_train, k, d_min, date)
