...
def Train_adv_network(model, network, device, train_loader, k, dmin, date):
    ...
    if isinstance(network, torch.nn.DataParallel):
        torch.save(network.module, '../offline/adv_fcnn_black/ut_adv_black_fcnn' + '%d-' % k + '.pth')
    else:
        torch.save(network, '../offline/adv_fcnn_black/ut_adv_black_fcnn' + '%d-' % k + '.pth')
    return network
...
for k in np.arange(num_classes):
    ...
    # network = torch.load('../offline/adv_fcnn_black/ut_adv_black_fcnn' + '%d-' % k + '.pth')
...
# file_name = '../offline/conv_white/ut_Attack_Results_all_fcnn_black.mat'
